...
export const generateMetadata = async (
  ...
  return {
    ...
    openGraph: {
      ...
      images: [
        {
          url: `${site.url}/api/og?title=${post.title}&date=${post.date.split('T')[0]
            }&url=blog.jotacode.dev/blog`,
          width: 1200,
          height: 630,
          alt: post.title,
          type: 'image/png'
    ...
    twitter: {
      ...
      description: post.summary,
      images: [
        `${site.url}/api/og?title=${post.title}&date=${post.date.split('T')[0]
        }&url=blog.jotacode.dev/blog`
      ]
    }
  }
...
const BlogPostPage = (props: BlogPostPageProps) => {
  ...
  const jsonLd: WithContext<Article> = {
    ...
    image: `${site.url}/api/og?title=${title}&date=${date.split('T')[0]
      }&url=blog.jotacode.dev/blog`,
    author: {
      '@type': 'Person',
      name: site.name,
